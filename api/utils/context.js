...
const getContext = async (message, namespace, maxCharacters = 5000, minScore = 0.4, getOnlyText = true) => {
    try {
        console.log("Subject: ", message);
        console.log("Workspace: ", namespace);
        // Wrap the message in an array before passing it to embedChunks
        const embeddings = await (0, embeddings_1.embedChunks)([message]);
        // Extract the embedding from the response
        const embedding = embeddings[0].embedding;
        const matches = await (0, pinecone_1.getMatchesFromEmbeddings)(embedding, 15, namespace);
