...
class ModuleController {
    ...
    */
    async createModule(req, res) {
        ...
        try {
            ...
            await connection.execute('INSERT INTO module_Modules (ModuleID, Name, Description, WorkspaceID) VALUES (?, ?, ?, ?)', [moduleId, name, description ? description : '', workspaceId]);
            // Create a root module node or initial structure
            await connection.execute('INSERT INTO module_ModuleNodes (ModuleNodeID, Title, Content, ModuleID) VALUES (?, ?, ?, ?)', [moduleId, '', '', moduleId]);
            await connection.execute('INSERT INTO module_ModuleClosureTable (ModuleID, Ancestor, Descendant, Depth, Position) VALUES (?, ?, ?, 0, 0)', [moduleId, moduleId, moduleId]);
    ...
    */
    async createModuleCallback(name, description, workspaceId, createdModuleId) {
        try {
            const connection = await (0, database_1.getDbConnection)();
            const moduleId = createdModuleId ? createdModuleId : (0, uuid_1.v4)();
            // Create the new module
            await connection.execute('INSERT INTO module_Modules (ModuleID, Name, Description, WorkspaceID) VALUES (?, ?, ?, ?)', [moduleId, name, description, workspaceId]);
            // Create a root module node or initial structure
            await connection.execute('INSERT INTO module_ModuleNodes (ModuleNodeID, Title, Content, ModuleID) VALUES (?, ?, ?, ?)', [moduleId, '', '', moduleId]);
            await connection.execute('INSERT INTO module_ModuleClosureTable (ModuleID, Ancestor, Descendant, Depth, Position) VALUES (?, ?, ?, 0, 0)', [moduleId, moduleId, moduleId]);
            await connection.end();
    ...
    */
    async getModules(req, res) {
        ...
        try {
            const connection = await (0, database_1.getDbConnection)();
            const [rows] = await connection.execute(`SELECT * FROM module_Modules WHERE WorkspaceID = ? ORDER BY CreatedAt`, [workspaceId]);
            await connection.end();
     ...
     * @param res - The express response object.
     */
    async updateModuleName(req, res) {
        const token = req.cookies.authToken;
        // Check for the token
        if (!token) {
        ...
        try {
            ...
            await connection.execute(`UPDATE module_Modules SET Name = ? WHERE WorkspaceID = ? AND ModuleID = ?`, [newName, workspaceId, moduleId]);
            await connection.end();
    ...
    async deleteModule(req, res) {
        ...
        try {
            ...
            const result = await connection.execute(`DELETE FROM module_Modules WHERE ModuleID = ?`, // Assuming the column name is `id`
            [moduleId]);
    ...
    * @param res The response object.
    */
    async insertChildToModuleNode(req, res) {
        if (req.method !== 'POST') {
            return res.status(405).json({ message: 'Method Not Allowed' });
        }
        ...
        try {
            ...
            const [moduleExists] = await connection.execute('SELECT COUNT(*) AS count FROM module_Modules WHERE ModuleID = ?', [moduleId]);
            const [parentNodeExists] = await connection.execute('SELECT COUNT(*) AS count FROM module_ModuleNodes WHERE ModuleNodeID = ?', [parentNodeId]);
            ...
            const moduleNodeID = (0, uuid_1.v4)();
            // Insert new module node as a child
            await connection.execute('INSERT INTO module_ModuleNodes (ModuleNodeID, Title, Content, ModuleID) VALUES (?, ?, ?, ?)', [moduleNodeID, title, content, moduleId]);
            // Calculate the new position as the count of current siblings
            const [positionData] = await connection.execute(`SELECT COUNT(*) AS siblingCount FROM module_ModuleClosureTable 
        WHERE Ancestor = ? AND Depth = (SELECT Depth + 1 FROM module_ModuleClosureTable WHERE Descendant = ? AND ModuleID = ?) 
        AND ModuleID = ?`, [parentNodeId, parentNodeId, moduleId, moduleId]);
            ...
            const [depthResult] = await connection.execute(`SELECT Depth FROM module_ModuleClosureTable WHERE Descendant = ? AND ModuleID = ?`, [parentNodeId, moduleId]);
            const newDepth = depthResult[0].Depth + 1;
            // Now perform the insert
            await connection.execute(`INSERT INTO module_ModuleClosureTable (ModuleID, Ancestor, Descendant, Depth, Position)
        VALUES (?, ?, ?, ?, ?)`, [moduleId, parentNodeId, moduleNodeID, newDepth, newPosition]);
            (0, charge_1.chargeUserByModuleId)(connection, content, moduleId);
            await connection.end();
    ...
    */
    async insertChildToModuleNodeCallback(parentNodeId, moduleId, moduleNodeId, content, title, position, depth) {
        try {
            const connection = await (0, database_1.getDbConnection)();
            // Check if the module and parent node exist
            const [moduleExists] = await connection.execute('SELECT COUNT(*) AS count FROM module_Modules WHERE ModuleID = ?', [moduleId]);
            ...
            // Perform the insert
            await connection.execute(`INSERT INTO module_ModuleClosureTable (ModuleID, Ancestor, Descendant, Depth, Position)
        VALUES (?, ?, ?, ?, ?)`, [moduleId, parentNodeId, moduleNodeID, depth, position]);
            await connection.end();
    ...
    */
    async getModuleTree(req, res) {
        ...
        try {
            const connection = await (0, database_1.getDbConnection)();
            const [moduleRows] = await connection.execute(`SELECT WorkspaceID FROM module_Modules WHERE ModuleID = ?`, [moduleId]);
            ...
            const existingTreeData = index_1.default.getInstance().socketServer.workspaceModulesBuffer.get(serializedKey);
          ...
          WHERE 
            mc.ModuleID = ?`, [moduleId]);
                ...
                if (!tree)
                    throw new Error("Module Tree does not exist.");
                await connection.end();
                res.status(200).json({
                    WorkspaceID: workspaceId,
    ...
    */
    async getSubtree(req, res) {
        ...
        try {
            const connection = await (0, database_1.getDbConnection)();
            const [rows] = await connection.execute(`SELECT * FROM module_ModuleClosureTable WHERE ModuleID = ?`, [moduleId]);
    ...
    */
    async fetchChildren(connection, moduleId, parentId) {
        // Fetch all descendants of the parent node
        const children = await connection.execute(`SELECT * FROM module_ModuleClosureTable
        WHERE ModuleID = ? AND Ancestor = ? AND Ancestor != Descendant
        ORDER BY Position`, [moduleId, parentId]);
    ...
    * @param res - The express response object.
    */
    async getSubtreeRecursively(req, res) {
        const { moduleId, moduleNodeId } = req.params;
        if (!moduleId || !moduleNodeId) {
            return res.status(400).json({ message: "Module ID and Node ID are required" });
        }
        try {
            ...
            const rootNode = await connection.execute(`SELECT * FROM module_ModuleClosureTable
         WHERE ModuleID = ? AND Descendant = ?`, [moduleId, moduleNodeId]);
    ...
    * @param res The response object.
    */
    async updateModuleNodeContent(req, res) {
        if (req.method !== 'PATCH') {
            return res.status(405).json({ message: 'Method Not Allowed' });
        }
        ...
        try {
            ...
            await connection.execute('UPDATE module_ModuleNodes SET Content = ? WHERE ModuleNodeID = ?', [content, moduleNodeId]);
            (0, charge_1.chargeUserByModuleId)(connection, content, moduleNodeId);
            await connection.end();
    ...
    */
    async updateModuleNodeContentCallback(moduleNodeId, content) {
        try {
            const connection = await (0, database_1.getDbConnection)();
            // Update content in the module node
            await connection.execute('UPDATE module_ModuleNodes SET Content = ? WHERE ModuleNodeID = ?', [content, moduleNodeId]);
    ...
    * @param res The response object.
    */
    async updateModuleNodeTitle(req, res) {
        if (req.method !== 'PATCH') {
            return res.status(405).json({ message: 'Method Not Allowed' });
        }
        ...
        try {
            ...
            await connection.execute('UPDATE module_ModuleNodes SET Title = ? WHERE ModuleNodeID = ?', [title, moduleNodeId]);
            await connection.end();
    ...
    */
    async updateModuleNodeTitleCallback(moduleNodeId, title) {
        try {
            const connection = await (0, database_1.getDbConnection)();
            // Update title in the module node
            await connection.execute('UPDATE module_ModuleNodes SET Title = ? WHERE ModuleNodeID = ?', [title, moduleNodeId]);
     ...
     * @param res The response object.
     */
    async deleteModuleNode(req, res) {
        if (req.method !== 'DELETE') {
            return res.status(405).json({ message: 'Method Not Allowed' });
        }
        ...
        try {
            ...
            const [nodeRows] = await connection.execute('SELECT Position, Depth, ModuleID FROM module_ModuleClosureTable WHERE Descendant = ?', [moduleNodeId]);
            ...
            await connection.execute('UPDATE module_ModuleClosureTable SET Position = Position - 1 WHERE ModuleID = ? AND Depth = ? AND Position > ?', [moduleId, deletedNodeDepth, deletedNodePosition]);
            // Step 3: Delete the module node
            await connection.execute('DELETE FROM module_ModuleClosureTable WHERE Descendant = ?', [moduleNodeId]);
            await connection.end();
    ...
    */
    async deleteModuleNodeCallback(moduleNodeId) {
        try {
            const connection = await (0, database_1.getDbConnection)();
            // Delete the module node
            await connection.execute('DELETE FROM module_ModuleNodes WHERE ModuleNodeID = ?', [moduleNodeId]);
            await connection.end();
