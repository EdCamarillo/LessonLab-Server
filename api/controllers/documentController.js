...
const storage_1 = require("../../src/utils/storage/storage");
const serverStorage_1 = require("../../src/utils/storage/serverStorage");
const spacesStorage_1 = require("../../src/utils/storage/spacesStorage");
class DocumentController {
    ...
    async addDocuments(req, res) {
        (0, multer_2.upload)(req, res, async (err) => {
            ...
            const workerPromises = files.map((file) => {
                return new Promise((resolve, reject) => {
                    ...
                    const documentUrl = storage_1.storageService.constructFileUrl(fileKey);
                    const workerPath = path_1.default.join(__dirname, "../utils/workers/fileProcessorWorker");
            ...
            try {
                ...
                await Promise.all(documents.map((document) => this.safeUpsertDocument(document, namespaceId)));
    ...
    async listFilesInNamespace(req, res) {
        ...
        try {
            let files;
            files = await storage_1.storageService.listFilesInNamespace(namespaceId);
    ...
    async deleteDocument(req, res) {
        ...
        try {
            ...
            do {
                try {
                    const listResult = await this.documentModel.listDocumentChunks(documentId, namespaceId, pageSize, paginationToken);
        ...
        try {
            ...
            var result = await storage_1.storageService.deleteFileFromWorkspace(namespaceId, documentId);
    ...
    async serveDocument(req, res) {
        ...
        try {
            const fileUrl = storage_1.storageService.constructFileUrl(fileKey);
            if (storage_1.storageService instanceof serverStorage_1.ServerStorage) {
                // Serve the file from the local filesystem
                const filePath = path_1.default.join("uploads", fileKey);
                const files = fs_1.default.readdirSync(filePath);
                const firstFile = files[0];
                const fileFullPath = path_1.default.join(filePath, firstFile);
                res.sendFile(fileFullPath, { root: "." });
            }
            else if (storage_1.storageService instanceof spacesStorage_1.SpacesStorage) {
